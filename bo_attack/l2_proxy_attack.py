...
from utils import proj_l2, latent_proj, transform
...
def obj_func_proxy(x, x0, y0):
    ...
    # TODO: Check if norm of x is held after transform.
    x = proj_l2(x, args.eps)

    with torch.no_grad():
        d, perturb, n_query = fine_grained_binary_search_local(cnn_model, x0, y0, x)

    if args.verbose:
        print(f"Distance: {d}, norm: {perturb.norm().item()}, {n_query} queries taken.")
...
def bayes_opt(x0, y0, tol=1e-5):
    ...
    while query_count < args.iter:
        ...
        best_norm = best_candidate.norm().item()
